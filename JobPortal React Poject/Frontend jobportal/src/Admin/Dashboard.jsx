...
const Dashboard = () => {
  ...
  const fetchTotalDeclinedApplications = async () => {
    try {
      const response = await fetch(
        "http://localhost:8070/applications/declined/total"
      );
      if (!response.ok) {
  ...
  const fetchTotalPendingApplications = async () => {
    try {
      const response = await fetch(
        "http://localhost:8070/applications/pending/total"
      );
      if (!response.ok) {
    ...
    <div>
      ...
      <div className="pt-[25px] px-[25px] bg-[#F8F9FC]">
        <div className="flex items-center justify-between">
          <h1 className="text-[#2E59D9] text-[28px] leading-[34px] cursor-pointer font-semibold">
            Dashboard
          </h1>
        </div>
        ...
        {/* User Info */}
        <div>
          <h1 className="text-[#7c7c7c] font-bold pt-5 text-[22px]">
            User Info
          </h1>
          <div className="grid grid-cols-4 gap-[30px] mt-[25px] pb-[15px]">
            <div className="h-[120px] rounded-[8px] border-2 border-[#0080FF] flex items-center justify-between px-[30px] cursor-pointer hover:shadow-3xl transform hover:scale-[103%] transition duration-300 ease-out bg-[#0080FF]">
              <div>
                <h2 className="text-white text-[15px] leading-[17px] font-bold">
                  Total Members
                </h2>
