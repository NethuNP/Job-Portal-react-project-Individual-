...
export default function JobCategory() {
    ...
    useEffect(() => {
        function getFeedback() {
            axios.get("http://localhost:8070/fedbacks/")
                .then((res) => {
    ...
    const closeReplyPopup = () => {
        ...
        setReplyContent(""); // Reset reply content when closing popup
    };
            ...
            <div className="container mx-auto xl:px-30 px-4 bg-white mt-20 h-full w-full pb-10">
                <div className="py-4 px-10">
                                        ...
                                        <tbody>
                                            {currentFeedback.map((feedbackItem, index) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-6 py-4 text-center">{feedbackItem.name}</td>
                                                    <td className="px-6 py-4 text-center">{feedbackItem.email}</td>
                                                    <td className="px-6 py-4 text-center">{feedbackItem.message}</td>
                                                    <td className="px-6 py-4 text-center">{feedbackItem.reply}</td>
                                                    <td className="px-6 py-4 text-center flex justify-center">
                                                        ...
                                                        <button
                                                            onClick={() => openReplyPopup(feedbackItem, indexOfFirstItem + index)}
                    ...
                    <div className="flex justify-center mt-4 space-x-8 text-blue">
                        ...
                        <button
                            onClick={nextPage}
                            disabled={currentPage === Math.ceil(feedback.length / itemsPerPage)}
                            className="hover:underline"
                        >
                            Next
                        </button>
            ...
            {showReplyPopup && (
                <div className="fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-50 flex justify-center items-center">
                    <div className="bg-white p-8  shadow-xl w-96 rounded-3xl">
                        <h2 className="text-xl mb-4 text-white font-bold pl-28 bg-[#2c42a5] dark:bg-gray-900 rounded-full py-2">Send Reply</h2>
                        <form onSubmit={(e) => {
                            e.preventDefault();
                            submitReply();
                        }}>
