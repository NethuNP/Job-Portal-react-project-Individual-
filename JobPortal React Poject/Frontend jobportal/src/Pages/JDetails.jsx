      ...
      {approvedJob && (
        <>
          <div>
            {user ? (
              <button
                className="bg-blue text-white px-4 py-3 mt-10 rounded-md"
              ...
              <div className="text base text-primary font-medium space-x-5 hidden lg:block">
                <Link
                  to="/login"
                  className="py-2 px-5 bg-blue text-white border-2 border-blue rounded font-bold hover:text-blue hover:bg-white mt-10"
                >
                  Register For Apply
                </Link>
              </div>
          ...
          <div className="bg-[#b8ccf1] md:grid grid-cols-3 gap-5 lg:px-24 px-4 py-12 mt-3 h-auto mb-10 rounded-3xl shadow-3xl">
            <div className="bg-white p-4 rounded shadow-3xl">
              ...
              <div className="flex items-center mb-4 justify-center">
                <span className="font-semibold text-xl bg-gray-200 text-black py-2 border-2 rounded-full  hover:bg-gray-400 px-5 hover:text-white">
                  {approvedJob.companyName}
                </span>
              </div>
              <div className="flex items-center mb-4">
                ...
                <h1 className="font-semibold">
                  {" "}
                  Title: {approvedJob.jobTitle}
                </h1>
              </div>
              <div className="flex items-center mb-4">
                <IoBagRemoveSharp className="mr-2 text-[20px]" />
                <span className="font-semibold">
                  Job Type: {approvedJob.employmentType}
                </span>
              </div>
              <div className="flex items-center mb-4">
                <RiMapPin2Fill className="mr-2 text-[20px]" />
                <span className="font-semibold">
                  Location: {approvedJob.jobLocation}
                </span>
              </div>
              <div className="flex items-center mb-4">
                <FaMoneyCheckDollar className="mr-2 text-[20px]" />
                <span className="font-semibold">
                  Salary: Rs: {approvedJob.minPrice} - {approvedJob.maxPrice}
                </span>
              </div>
            </div>
            ...
            {showUploadSection && (
              <div className="bg-white p-4 rounded shadow-3xl col-span-full mt-5">
                ...
                <div
                  className="drag-area h-[150px] border-r-[2px] border-[2px] border-dashed border-blue bg-white flex justify-center items-center select-none mt-4"
                ...
                >
                  {isDragging ? (
                    ...
                    <>
                      ...
                      <span
                        className="select text-purple-800 ml-[8px] text-2xl font-bold cursor-pointer transition:0.4s hover:opacity-[0.6]"
                        role="button"
                        onClick={selectFile}
                      >
                        Browse
                ...
                <div className="container w-[100%] h-auto flex justify-start items-start flex-wrap max-h-[200px] overflow-y-auto mt-[10px]">
                  {files.map((file, index) => (
                    <div
                      className="image w-[75px] mr-[5px] h-[75px] relative mb-[8px]"
                      key={index}
                    >
                      <span
                        className="delete absolute top-[-2px] r-[9px] font-[20px] cursor-pointer z-[999] text-blue"
                      ...
                      <img
                        src={file.url}
                        alt={file.name}
                        className="w-[100%] h-[100%] border-r-[5px] img-display "
                      />
                    </div>
                  ))}
                </div>
                <button
                  type="button"
                  className="w-[100%] p-[8px 13px] font-bold cursor-pointer border-0 outline-0 border-[#fff] bg-blue text-white rounded-lg py-1"
                  onClick={uploadFile}
                >
                  Upload
                </button>
