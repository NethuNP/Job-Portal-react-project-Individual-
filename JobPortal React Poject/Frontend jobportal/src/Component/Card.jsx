...
const Card = ({ data }) => {
  ...
  return (
    ...
    >
      <Link
        to={`/jobs/${_id}`}
        className="flex gap-4 flex-col sm:flex-row items-start m-5 border-solid bg-slate-100 cursor-pointer drop-shadow-xl hover:bg-slate-300"
      >
        <motion.img
          src={companyLogo}
          alt=""
        ...
        <div>
          ...
          <div className="text-primary/70 text-base flex flex-wrap gap-10 mb-2">
            <span className="flex items-center gap-2">
              <FaLocationDot /> {jobLocation}
            </span>
            <span className="flex items-center gap-2 ">
              {" "}
              <FiDollarSign className="text-black font-bold" />
